...
public class VideoMapper {
  ...
  public VideoDto toDto(final VideoDao videoDao) {
    return new VideoDto(
        ...
        actorRepository.findByVideos_Id(videoDao.getId()).stream().map(actorMapper::toDto).toList(),
        videoDao.getStatus(),
  ...
  public VideoDao toEntity(final VideoDto videoDto) {
    ...
    videoDao.setFolderId(
        folderRepository
            .findByPath((videoDto.folder().path()))
    ...
    videoDao.setActors(
        videoDto.actors().stream()
            .map(
                actor ->
                    actorRepository.findByName(actor.name()).orElseThrow(RuntimeException::new))
